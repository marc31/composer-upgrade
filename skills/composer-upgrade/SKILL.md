...
- Keep the main table controls: arrow keys move, Enter/Space toggle selection, `i` opens changelog, `v` opens versions, `s` shows the plan, `x` requests execution, `d` runs dry run, `w` toggles the default-enabled `--with-all-dependencies`, and `q` requests exit confirmation. In that confirmation, `q`, Enter, and `y` quit; `n` cancels.
