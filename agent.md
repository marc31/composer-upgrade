...
- Keep the Textual interaction model consistent: table navigation uses arrow keys; `i` opens changelog, `v` opens versions, `s` shows the plan, `x` requests execution, `d` requests a dry run, `w` toggles the default-enabled `--with-all-dependencies`, and `q` opens the exit confirmation. In that confirmation, `q`, Enter, and `y` quit; `n` cancels.
